...
class plgEventlistMailer extends JPlugin {
	...
	public function onEventUserRegistered($register_id)
	{
		...
		$link = JRoute::_(JURI::base().EventListHelperRoute::getRoute($event->slug), false);
...
/**
	...
	public function onUserOnOffWaitinglist($register_id)
	{
		...
		$link =JRoute::_($url. EventlistHelperRoute::getRoute($details->slug), false);
		...
		if ($details->waiting) // added to the waiting list
		{
			...
			if ($this->params->get('reg_mail_user_onoff', '1')) {
				...
				$data 				= new stdClass();
				$data->subject 		= JText::sprintf('PLG_EVENTLISTMAILER_MAIL_USER_REG_ON_WAITING_SUBJECT', $this->_SiteName);
				$data->body			= JText::sprintf('PLG_EVENTLISTMAILER_MAIL_USER_REG_ON_WAITING_BODY', $user->name, $user->username, $details->title, $link, $SiteName);
				$data->receivers 	= $user->email;
			...
			if ($this->params->get('reg_mail_admin_onoff', '0') && $this->params->get('receivers')) {
				...
				$data 				= new stdClass();
				$data->subject 		= JText::sprintf('PLG_EVENTLISTMAILER_MAIL_ADMIN_REG_ON_WAITING_SUBJECT', $this->_SiteName);
				$data->body			= JText::sprintf('PLG_EVENTLISTMAILER_MAIL_ADMIN_REG_ON_WAITING_BODY', $user->name, $user->username, $details->title, $link, $SiteName);
				$data->receivers 	= $this->_receivers;
		...
		else // bumped from waiting list to attending list
		{
			...
			if ($this->params->get('reg_mail_user_onoff', '1')) {
				...
				$data 				= new stdClass();
				$data->subject 		= JText::sprintf('PLG_EVENTLISTMAILER_MAIL_USER_REG_ON_ATTENDING_SUBJECT', $this->_SiteName);
				$data->body			= JText::sprintf('PLG_EVENTLISTMAILER_MAIL_USER_REG_ON_ATTENDING_BODY', $user->name, $user->username, $details->title, $link, $SiteName);
				$data->receivers 	= $user->email;
			...
			if ($this->params->get('reg_mail_admin_onoff', '0') && $this->params->get('receivers')) {
				...
				$data 				= new stdClass();
				$data->subject 		= JText::sprintf('PLG_EVENTLISTMAILER_MAIL_ADMIN_REG_ON_ATTENDING_SUBJECT', $this->_SiteName);
				$data->body			= JText::sprintf('PLG_EVENTLISTMAILER_MAIL_ADMIN_REG_ON_ATTENDING_BODY', $user->name, $user->username, $details->title, $link, $SiteName);
				$data->receivers 	= $this->_receivers;
