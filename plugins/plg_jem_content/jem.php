...
class plgContentJem extends JPlugin
{
	 ...
	 * Event is passed by reference, but after the save, so no changes will be saved.
	 ...
	 *
	 * @todo: change code
	 * When selecting category's to delete it can happen that category's without events
	 * are not being deleted. Like cat3:1 event), cat2:(0 event), try to delete both.
	 *
	...
	public function onContentBeforeDelete($context, $data)
	{
		...
		$tableInfo = array (
				'com_jem' => array('table_name' => '#__jem_events'),
		);
		...
		if (isset($tableInfo[$extension]))
		{
			...
			else
			{
				...
				if ($count > 0 ) {
					$msg = JText::sprintf('COM_JEM_CATEGORIES_DELETE_NOT_ALLOWED', $data->get('catname')) .
					JText::plural('COM_JEM_CATEGORIES_N_ITEMS_ASSIGNED', $count);
				...
				if (!$data->isLeaf()) {
					...
					elseif ($count)
					{
						...
						$msg =
						//JText::sprintf('COM_JEM_CATEGORIES_DELETE_NOT_ALLOWED', $data->get('catname')) .
