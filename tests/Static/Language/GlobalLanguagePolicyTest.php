...
final class GlobalLanguagePolicyTest extends TestCase
{
    ...
    public function testBundledEnglishProvidesNewStatusInBothClients(): void
    {
        foreach (array(
            ...
            self::assertStringContainsString('COM_JEM_EVENT_STATUS_SCHEDULED="Scheduled"', $language);
            self::assertStringContainsString('COM_JEM_EVENT_STATUS_NEW="New"', $language);
        }
    }
    ...
    public function testRuntimeUsesGlobalLanguagesBeforeLegacyLocalFallbacks(): void
    {
        ...
        self::assertStringContainsString("\$language->load(\$extension, \$path, 'en-GB', true, false)", $helper);
        self::assertStringContainsString('$language->load($extension, $path, $tag, true, false)', $helper);
        self::assertMatchesRegularExpression(
            '#if \(\$isAdministrator\).*?\$siteLoaded = \$loadSite\(\);.*?\$adminLoaded = \$loadAdmin\(\);'
            . '.*?else.*?\$adminLoaded = \$loadAdmin\(\);.*?\$siteLoaded = \$loadSite\(\);#s',
            $helper
        );
        ...
        self::assertStringContainsString(
            "JemHelper::loadExtensionLanguage('mod_jem_types', JPATH_SITE",
            $this->read('modules/mod_jem_types/mod_jem_types.php')
        );
    ...
    public function testPackageUpdateCleansOnlyBundledLocalEnglishFiles(): void
    {
        ...
        self::assertSame(17, substr_count($installer, "/language/en-GB' =>"));
        self::assertStringContainsString('Other language tags belong to separately installed JEM language packs.', $installer);
        self::assertStringContainsString('Joomla override directories are never', $installer);
        self::assertStringNotContainsString('/language/overrides/', $installer);
        ...
        self::assertLessThan(
            ...
            strpos($postflight[0], '$this->normaliseJemModuleParams();')
        );
    }
