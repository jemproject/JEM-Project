...
abstract class modJEMbannerHelper
{
	...
	public static function getList(&$params)
	{
		...
		foreach ($events as $row)
		{
			...
			if ($access || in_array($row->access, $authorised)) {
				...
				$lists[$i]->linkText = JText::_('BANNER');
			}
			...
			# time/date
			$lists[$i]->date        = modJEMbannerHelper::_format_date($row, $params);
			$lists[$i]->day         = modJEMbannerHelper::_format_day($row, $params);
			$lists[$i]->dayname     = modJEMbannerHelper::_format_dayname($row);
			$lists[$i]->daynamefull = modJEMbannerHelper::_format_daynamefull($row);
			$lists[$i]->daynum      = modJEMbannerHelper::_format_daynum($row);
			$lists[$i]->month       = modJEMbannerHelper::_format_month($row);
			$lists[$i]->year        = modJEMbannerHelper::_format_year($row);
			$lists[$i]->time        = $row->times ? modJEMbannerHelper::_format_time($row->dates, $row->times, $params) : '' ;
	...
	protected static function _format_date($row, &$params)
	{
		...
		if ($params->get('datemethod', 1) == 2) {
			...
			//means a currently running event
				...
				$days = round(($today_stamp - $dates_stamp) / 86400);
				$result = JText::sprintf('MOD_JEM_BANNER_STARTED_DAYS_AGO', $days);
			...
			if (($dates_stamp > $tomorrow_stamp) && $enddates_stamp) {
				...
				$result = JText::sprintf('MOD_JEM_BANNER_FROM_UNTIL', $startdate, $enddate);
			}
	...
	protected static function _format_dayname($row)
	{
		...
		$dayname = $jdate->format('D', false, true);
		return $dayname;
	}

	protected static function _format_daynamefull($row)
	{
		$jdate	= new JDate($row->dates);
		$daynamefull = $jdate->format('l', false, true);
		return $daynamefull;
	}
