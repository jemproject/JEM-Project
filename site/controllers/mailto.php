...
class JemControllerMailto extends JemControllerForm
{
	...
	public function save($key = NULL, $urlVar = NULL){
		...
		$subject_default = JText::sprintf('COM_JEM_MAILTO_SENT_BY', $data['sender']);
		$subject         = $data['subject'] !== '' ? $data['subject'] : $subject_default;
		$error = false;

		if (!$data['emailto'] || !JMailHelper::isEmailAddress($data['emailto']))
		{
			$error = JText::sprintf('COM_JEM_MAILTO_EMAIL_INVALID', $data['emailto']);
		...
		{
			$error = JText::sprintf('COM_JEM_MAILTO_EMAIL_INVALID', $data['emailfrom']);
		...
		$msg  = JText::_('COM_JEM_MAILTO_EMAIL_MSG');
