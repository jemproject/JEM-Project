...
<div id="jem" class="jem_select_contact">
	...
	<form action="<?php echo JRoute::_('index.php?option=com_jem&view=editevent&layout=choosecontact&tmpl=component&function='.$this->escape($function).'&'.JSession::getFormToken().'=1'); ?>" method="post" name="adminForm" id="adminForm">
      ...
      <div id="jem_filter" class="jem-form jem-row jem-justify-start">
        ...
        <div class="jem-row jem-justify-start jem-nowrap">
          ...
          <button type="button" class="pointer btn btn-primary" onclick="if (window.parent) window.parent.<?php echo $this->escape($function);?>('', '<?php echo JText::_('COM_JEM_SELECT_CONTACT') ?>');"><?php echo JText::_('COM_JEM_NOCONTACT')?></button>
        </div>
      </div>
      <div class="jem-row jem-justify-start jem-nowrap">
        <div>
    ...
    <div class="jem-sort jem-sort-small">
      <div class="jem-list-row jem-small-list">
        <div class="sectiontableheader jem-contact-number"><?php echo JText::_('COM_JEM_NUM'); ?></div>
    ...
    <ul class="eventlist eventtable">
      <?php if (empty($this->rows)) : ?>
        <li class="jem-event jem-list-row jem-small-list"><?php echo JText::_('COM_JEM_NOCONTACTS'); ?></li>
      <?php else :?>
        <?php foreach ($this->rows as $i => $row) : ?>
          <li class="jem-event jem-list-row jem-small-list row<?php echo $i % 2; ?>">
            ...
            <div class="jem-event-info-small jem-contact-name">
              <span <?php echo JEMOutput::tooltip(JText::_('COM_JEM_SELECT'), $row->name, 'editlinktip selectcontact'); ?>>
