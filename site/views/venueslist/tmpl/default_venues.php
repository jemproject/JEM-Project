...
if (!function_exists('jem_venueslist_default_venue_page_link')) {
    function jem_venueslist_default_venue_slug($row)
    {
    ...
    {
        $slug = jem_venueslist_default_venue_slug($row);
        ...
        <thead>
            <tr>
                <?php foreach ($displayOrder as $field) : ?>
                    ...
                    <?php elseif ($field === 'state') : ?>
                        <th id="jem_state" class="sectiontableheader" style="text-align: left;"><i class="fa fa-map" aria-hidden="true"></i>&nbsp;<?php echo HTMLHelper::_('grid.sort', 'COM_JEM_VENUESLIST_TABLE_STATE', 'a.state', $this->lists['order_Dir'], $this->lists['order']); ?></th>
                    <?php elseif ($field === 'country') : ?>
                    ...
                    <?php foreach ($displayOrder as $field) : ?>
                        ...
                        <?php elseif ($field === 'venue') : ?>
                            <td headers="jem_location" style="text-align: left; vertical-align: middle;">
                                ...
                                echo $venueaccess;
                                ?>
                                <meta itemprop="identifier" content="<?php echo rtrim($uri->base(), '/').Route::_(JemHelperRoute::getVenueRoute(jem_venueslist_default_venue_slug($row))); ?>" />
