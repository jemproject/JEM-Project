    ...
    protected function getListQuery()
    {
        ...
        $query->select(
            $this->getState('list.select',
                'a.access,a.alias,a.attribs,a.checked_out,a.checked_out_time,a.contactid,a.created,a.created_by,a.created_by_alias,a.custom1,a.custom2,a.custom3,a.custom4,a.custom5,a.custom6,a.custom7,a.custom8,a.custom9,a.custom10,a.dates,a.datimage,a.fullimage,a.image_path,a.enddates,a.endtimes,a.featured,' .
        ...
        #############################
        $calendar_from    = $this->getState('filter.calendar_from');
        $calendar_to      = $this->getState('filter.calendar_to');
                ...
                if($applyCalendarDateFilters) {
                    if (empty($jubilee_filter) || !$jubilee_show_past) {
                        if ($cal_month) {
                            ...
                            // Apply menu parameters date filters
                            // Retrieve from State to maintain consistency during pagination
                            $filterDaysBefore = $this->getState('filter.tablefiltereventfrom');
                            if ($filterDaysBefore === null) {
                                $filterDaysBefore = isset($params) ? $params->get('tablefiltereventfrom', '') : '';
                            }

                            $filterDaysAfter = $this->getState('filter.tablefiltereventuntil');
                            if ($filterDaysAfter === null) {
