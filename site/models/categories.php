...
class JemModelCategories extends BaseDatabaseModel
{
	...
	protected function _buildDataQuery($id)
	{
		...
		$query = 'SELECT DISTINCT a.id, a.dates, a.enddates, a.times, a.endtimes, a.title, a.locid, a.created, a.published, a.access,'
			   ...
			   . ' CASE WHEN a.access IN (' . implode(',',$levels) . ') THEN 1 ELSE 0 END as user_has_access'
	...
	protected function _buildQuery($emptycat, $parent_id = null, $parentCategory = false)
	{
		...
		$case_when_a  = ' CASE WHEN c.access IN (' . implode(',',$levels) . ') THEN 1 ELSE 0 END as user_has_access,';
