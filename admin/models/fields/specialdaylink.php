...
class JFormFieldSpecialdaylink extends FormField
{
    ...
    protected function getInput()
    {
        ...
        $articleLink = Uri::root(true) . '/administrator/index.php?option=com_content&view=articles&layout=modal&tmpl=component&function=' . $articleCallback . '&' . Session::getFormToken() . '=1';
        $menuLink = Uri::root(true) . '/administrator/index.php?option=com_menus&view=items&layout=modal&tmpl=component&function=' . $menuCallback . '&' . Session::getFormToken() . '=1';
        ...
        $script[] = '                var selected = event.target && event.target.closest ? event.target.closest("[data-content-select], .select-link") : null;';
        $script[] = '                if (!selected) { return; }';
        $script[] = '                event.preventDefault();';
        $script[] = '                setSpecialDayLink(source, selected.getAttribute("data-title") || selected.textContent || "", selected.getAttribute("data-uri") || "", selected.getAttribute("data-id") || "");';
        $script[] = '            }, true);';
        $script[] = '        };';
        ...
        $script[] = '        setSpecialDayLink("menu", title, link || arg3, id);';
        $script[] = '};';
        ...
        $script[] = '        setSpecialDayLink(source, data.title || "", data.uri || "", data.id || "");';
        $script[] = '    });';
