 ...
 * @since 0.9
 */
class EventListViewEvents extends JViewLegacy {
	...
	function display($tpl = null)
	{
		...
		$filter_state 		= $app->getUserStateFromRequest( 'com_eventlist.events.filter_state', 'filter_state', 	'*', 'word' );
		$filter 			= $app->getUserStateFromRequest( 'com_eventlist.events.filter', 'filter', '', 'int' );
		$search 			= $app->getUserStateFromRequest( 'com_eventlist.events.search', 'search', '', 'string' );
		...
		JToolBarHelper::spacer();
		JToolBarHelper::deleteList($msg = 'COM_EVENTLIST_CONFIRM_DELETE', $task = 'remove', $alt = 'JACTION_DELETE');
		JToolBarHelper::spacer();
